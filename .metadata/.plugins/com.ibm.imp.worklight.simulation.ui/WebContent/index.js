...
require([
        ...
        "dijit/MenuItem",
        "dijit/Menu",
        "dijit/form/DropDownButton"], function(dojo, dijit, ready, has, sniff, dom, win, html, connect, arr, WidgetRegistry, xhr, lang, json, bwin, query, number, parser, on, domStyle,
        domConstruct, domGeom, CheckBox, Dialog){
	...
	var isBrowserCordovaCapable = function(){
		return has("chrome") || has("safari") || has("ff");
	};
	...
	var getCordovaCapableBrowsersStrings = function(){
		...
		return [nls.Google_Chrome, nls.Safari, nls.Mozilla_Firefox];
	};
	...
	var createGrid = function() {
		...
		addBtn.placeAt('addDeviceBlock', 'first');
	...
	var enableCordova = function(mode){
		if (mode) {
			...
			loadScript("http://openlayers.org/api/2.10/OpenLayers.js", false, function(){
	...
	var configFile = function(){
		var xhrArgs = {
		    ...
		    load : function(data){
				...
				if (enabled == true) {
					var cordovaEnablementBlock = dojo.byId('_cordovaEnablementBlock');
					dojo.style(cordovaEnablementBlock, {
						display : "inline"
					});
				} 
	...
	ready(function(){
		...
		// add warning if IE version less than 9
		if (dojo.isIE < 9) {
			var warningBox = dojo.byId('_warningBox');
			dojo.style(warningBox, 'display', 'block');
			dojo.attr(warningBox, 'innerHTML', nls.index_scaling_warning);
		}
		...
		var deviceStoreDeferred = dojo.xhrGet({
		    ...
		    load : function(response, ioArgs){
			    ...
			    if (platformID != null) {
			    	...
			    	if (platformID.indexOf("windowsphone") != -1)
